// Copyright 2020 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Package jwks handles returning JSON encoded information about the
// server's encryptionn keys. See https://tools.ietf.org/html/rfc75170
// for information about the server.
package jwks

import (
	"context"
	"crypto"
	"errors"
	"net/http"
	"strconv"
	"time"

	"github.com/google/exposure-notifications-server/pkg/logging"
	"github.com/google/exposure-notifications-verification-server/pkg/database"
	"github.com/google/exposure-notifications-verification-server/pkg/keyutils"
	"github.com/google/exposure-notifications-verification-server/pkg/render"
	"github.com/gorilla/mux"
	"github.com/rakutentech/jwk-go/jwk"

	"go.uber.org/zap"
)

// Error codes
var errBadRealm = errors.New("bad realm")
var errBadKeyID = errors.New("bad keyid")
var errNoKey = errors.New("no key found")
var errEncode = errors.New("error encoding key")

type Controller struct {
	h        *render.Renderer
	db       *database.Database
	keyCache *keyutils.PublicKeyCache
	logger   *zap.SugaredLogger
}

// HandleIndex returns an http.Handler that handles jwks GET requests.
func (c *Controller) HandleIndex() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Grab the URL params we need.
		realmID := mux.Vars(r)["realm"]
		keyID := mux.Vars(r)["keyid"]

		// Find the realm, and the key abstractions from the DB.
		realm, err := c.getRealm(realmID)
		if err != nil {
			c.renderError(w, err)
			return
		}
		var key *database.SigningKey
		key, err = c.getKey(realm, keyID)
		if err != nil {
			c.renderError(w, err)
			return
		}

		// Get the key.
		ctx := r.Context()
		var pk crypto.PublicKey
		pk, err = c.keyCache.GetPublicKey(ctx, key.KeyID, c.db.KeyManager())
		if err != nil {
			c.renderError(w, errNoKey)
			return
		}

		// Encode it, and sent it off.
		spec := jwk.NewSpec(pk)
		spec.KeyID = key.GetKID()
		var encoded *jwk.JWK
		encoded, err = spec.ToJWK()
		if err != nil {
			c.renderError(w, errEncode)
			return
		}
		c.h.RenderJSON(w, http.StatusOK, encoded)
	})
}

// getRealm finds given realm, given it's ID.
func (c *Controller) getRealm(realmStr string) (*database.Realm, error) {
	realmID, err := strconv.Atoi(realmStr)
	if err != nil {
		return nil, errBadRealm
	}
	var realm *database.Realm
	realm, err = c.db.FindRealm(uint(realmID))
	if err != nil {
		return nil, errBadRealm
	}
	return realm, nil
}

// getKey finds the key of the given ID.
func (c *Controller) getKey(realm *database.Realm, keyStr string) (*database.SigningKey, error) {
	keyID, err := strconv.Atoi(keyStr)
	if err != nil {
		return nil, errBadKeyID
	}
	var keys []*database.SigningKey
	keys, err = realm.ListSigningKeys(c.db)
	if err != nil {
		return nil, errBadKeyID
	}
	for _, key := range keys {
		if key.ID == uint(keyID) {
			return key, nil
		}
	}
	return nil, errBadKeyID
}

// reportError renders an error.
func (c *Controller) renderError(w http.ResponseWriter, err error) {
	w.WriteHeader(http.StatusNotFound)
	w.Write([]byte(err.Error()))
}

// New creates a new jwks *Controller, and returns it.
func New(ctx context.Context, db *database.Database, h *render.Renderer) (*Controller, error) {
	kc, err := keyutils.NewPublicKeyCache(time.Minute)
	if err != nil {
		return nil, err
	}
	logger := logging.FromContext(ctx)
	return &Controller{
		h:        h,
		db:       db,
		keyCache: kc,
		logger:   logger,
	}, nil
}
